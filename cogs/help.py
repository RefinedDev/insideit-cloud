...
class helpcommand(Cog):
    ...
    async def help(self,ctx):
        if ctx.invoked_subcommand is None:
            ...
            em.add_field(name = "😂 Memes",value = '`?help memey`',inline=False)
    ...
    @help.command()
    async def memey(self,ctx):
        embed = discord.Embed(colour = ctx.author.color)
        await ctx.message.delete()
        embed.add_field(name = '😂 Meme Commands.',value = "`?meme`,`?dankmeme`,`?chan`,`?AntiJoke`,`?wholesome`,`?surreal`,`?facepalm`")
        await ctx.send(embed = embed)
