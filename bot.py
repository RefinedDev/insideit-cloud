    ...
    try:
        ...
        if (len(res) == 0):
            ...
            if channel != None:
                ...
                embed.add_field(name = 'We got a new member!',value = f'**{str.capitalize(member.name)}** has joined the server!')
...
async def on_member_remove(member):
    try:
        ...
        if (len(res) == 0):
            ...
            if channel != None:
                embed = discord.Embed(timestamp = datetime.utcnow(),color = discord.Colour.blue())
                embed.add_field(name = 'We lost one :(',value = f'**{str.capitalize(member.name)}** has left the server!')
                embed.set_thumbnail(url = member.avatar_url)
                embed.set_footer(text = f'We have {len(member.guild.members)} now!')
                await channel.send(embed = embed)
            else:
